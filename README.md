...
- Datasources
- Policies
- Interactive Workloads (not yet implemented)
- Training Workloads (not yet implemented)

> [!NOTE]  
> Important notes: <br />
> * This script does not apply migration to the default node pool and the default department at the moment. Please change them manually, before running the script. This includes changing references to their ids in any resources that might reference them that are not created by the script (such as PVCs or Secrets that reference the department ID in a label, relevant if you are migrating PVCs or Credentials). <br />
> * Make sure you also have an application API token for system administrator on tenant scope in each cluster <br />
> * If you are moving any Credentials resources and/or plan to copy PVCs from the old cluster to the new cluster, you will need to move those resources yourself, the script will not do that for you. Do not change the name of the resource when copying it from the old cluster to the new cluster, simply copy as is and the script will take care of updating the corresponding Datasource or Credential to reference it.<br />
>   * Note that while moving PVCs is only required if you want to retain the data in the PVCs (and must be flagged with the ```--convert-new-pvc-datasources-to-existing``` flag), the Secrets for Credentials resources (whether the Credential was created from an existing Secret or the Credential was created from scratch in the UI) MUST be moved, as you cannot retrieve all the details of a given Credential from the API, it is only stored encrypted in the Postgres DB and in the cluster itself as a Secret. <br />
> * If you have created Node Affinity Types (they will be in the `nodetypes.json` file output from `retrieve_json.py`), these cannot be created via the API in 2.18. You must go into the UI and create them manually by:
>   * Opening the "create project" or "edit project" page
>   * Going to the "scheduling rules" section at the bottom
>   * Temporarily creating a new rule of type "Node Affinity" and when prompted for the value of the type, add a new type with the name from the previous cluster.
>   * If you have multiple node affinity types, you must create them in the same order they were created in the old cluster (i.e. from `id` 1 onwards) in order for the projects to point to the correct node affinity types after the script is run.
>   * Once the new node type(s) are created, you can exit out of the create/edit project page without saving, the types will still be retained in the database.
<br />
